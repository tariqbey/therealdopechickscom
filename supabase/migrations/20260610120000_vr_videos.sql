...
-- Videos live in a PRIVATE bucket; fans get access via signed URLs only after
-- unlocking, so the paywall is enforced at the storage layer, not just the UI.
...
CREATE TABLE public.vr_videos (
  ...
  title TEXT NOT NULL,
  description TEXT,
  video_path TEXT NOT NULL,        -- object path inside the private 'vr-videos' bucket
  thumbnail_url TEXT,              -- public URL in 'vr-thumbnails'
  price_bread INTEGER NOT NULL DEFAULT 0 CHECK (price_bread >= 0),  -- 0 = free
...
ON public.vr_video_unlocks FOR SELECT
USING (has_role(auth.uid(), 'admin'::app_role));
-- No INSERT policy: unlocks are only written by the unlock_vr_video() function below.

-- Storage: private bucket for the videos, public bucket for thumbnails
INSERT INTO storage.buckets (id, name, public) VALUES ('vr-videos', 'vr-videos', false)
ON CONFLICT (id) DO NOTHING;
INSERT INTO storage.buckets (id, name, public) VALUES ('vr-thumbnails', 'vr-thumbnails', true)
ON CONFLICT (id) DO NOTHING;
...
-- The paywall: a fan can only read (and therefore sign a URL for) a VR video
-- object if they own it, are admin, it's free, or they've unlocked it.
...
BEGIN
  ...
  -- Creator earns 80%; platform keeps 20%
